...
bl_info = {
    ...
    "version": (1, 1, 6),
    "blender": (2, 80, 0),
    "location": "3D View > UI > Mixamo Tab",
...
class OBJECT_PT_MixamoconvPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        box = layout.box()
        row = box.row()
        row.prop(scn.mixamo, "advanced", toggle=True)

        # row.prop(scn.mixamo, "mode")
        if scn.mixamo.advanced:
            split = box.split()
            ...
            row = box.row()
            row.prop(scn.mixamo, "fixbind")
            row.prop(scn.mixamo, "apply_rotation")
            # row = box.row()
            # row.prop(scn.mixamo, "quaternion_clean_pre")
            # row.prop(scn.mixamo, "quaternion_clean_post")


            row = box.row()
            row.prop(scn.mixamo, "apply_scale")
            if scn.mixamo.apply_scale:
                row.prop(scn.mixamo, "scale")

            row = box.row()
            row.prop(scn.mixamo, "experimental", toggle=True, icon='ERROR')
            if scn.mixamo.experimental:
                row = box.row()
                row.operator("mixamo.convertsingle_stepwise")
                split = box.split()
                col = split.column()
                col.prop(scn.mixamo, "restoffset")

                col.operator("mixamo.apply_restoffset")
                col = split.column()
                col.prop(scn.mixamo, "knee_offset")
        ...
        #col = split.column()
        row = box.row()
        row.prop(scn.mixamo, "inpath")
        if scn.mixamo.advanced:
            row = box.row()
            row.prop(scn.mixamo, "ignore_leaf_bones")
            row.prop(scn.mixamo, "automatic_bone_orientation")

        row = box.row()
        row.prop(scn.mixamo, "outpath")
        if scn.mixamo.advanced:
        #     col = split.column()
        #     col.prop(scn.mixamo, "ignore_leaf_bones")
            row = box.row()
            row.prop(scn.mixamo, "add_leaf_bones")
            row.prop(scn.mixamo, "force_overwrite")
